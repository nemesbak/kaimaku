...
- **`kaimaku_cli.py`** — CLI por lotes para procesar una biblioteca entera de una vez (scan → search → stage → install → refresh), pensada para automatizar vía cron/user scripts.
...
`kaimaku_cli.py refresh` refresca Jellyfin/Emby a partir del listado de cambios que genera el comando `download` (`--changed`); el flujo `stage` de arriba no lo produce, así que tras instalar refresca las bibliotecas afectadas a mano (o usa la interfaz web, que sí refresca automáticamente cada instalación).
